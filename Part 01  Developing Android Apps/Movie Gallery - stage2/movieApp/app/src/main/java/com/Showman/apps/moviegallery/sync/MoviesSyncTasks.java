...
public class MoviesSyncTasks {
    ...
    synchronized public static void syncNewMovies(final Context context) {
        ...
        try {
            ...
            if (moviesList != null && moviesList.size() > 0) {
                ...
                Log.e("newAndCurrentCodes", "CurrentCode = " + existCode + ",   newCode " + newCode);
                AppExecutors.getsInstance().getMainThread().execute(new Runnable() {
                    @Override
                    public void run() {
                        Toast.makeText(context, "Task_Started", Toast.LENGTH_LONG).show();
                    }
                });
